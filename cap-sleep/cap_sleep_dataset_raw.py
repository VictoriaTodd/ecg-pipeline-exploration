...
class CapSleepDatasetRaw(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ecg_signal = raw_window.get_data()[0]
        current_len = ecg_signal.shape[0]
        expected_len = int(self.window_size * self.sfreq)
        if current_len < expected_len:
            ecg_signal = np.pad(ecg_signal, (0, expected_len - current_len), mode='constant')
        elif current_len > expected_len:
            ecg_signal = ecg_signal[:expected_len]

        # --------- LABEL EXTRACTION ---------
        # Get annotations overlapping with the current window
        overlapping = annotations.crop(tmin=start_sec, tmax=stop_sec)

        # Multi-class label strategy
        ...
        return torch.tensor(ecg_signal, dtype=torch.float32), torch.tensor(label, dtype=torch.long)
